...
def speedyGET(args):
    ...
    def filter_cone_greater_10(k):
        sk = str(k)
        if sk in asn_data_global:
            return asn_data_global[sk] > 10
        return False

    threads = []
    # nodeList = list(filter(filter_cone_greater_10,nodeList))
    # print(nodeList)
...
def run_routingTree(dsn_file, relaFile,_dst_dir_path):
    dataConverter(relaFile, os.path.join(_dst_dir_path,'temp','bgp-sas.npz'))
    maxNum = graphGenerator(os.path.join(_dst_dir_path,'temp','bgp-sas.npz'), os.path.join(_dst_dir_path,'temp','routingTree.mtx'))
    speedyGET(['',os.path.join(_dst_dir_path,'temp','routingTree.mtx'), 'v', dsn_file, os.path.join(_dst_dir_path,'temp','routingTree.mtx.nodeList'), str(maxNum)])


'''生成一个国家内的排名前10的AS的路由树'''
def rTree(relas, dsn_file,_dst_dir_path):
    
    relaFile = os.path.join(dsn_file, 'as-rel.txt')
    #if len(relas)==0:
    #    return
    create_rela_file(relas, relaFile)
    run_routingTree(dsn_file, relaFile,_dst_dir_path)
...
def monitor_routingTree(file,dsn_path,country,as_rela_file,_dst_dir_path):
    ...
    rTree(relas, dsn_path,_dst_dir_path)
    print(country+' end')
