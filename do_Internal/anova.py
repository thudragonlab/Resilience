...
def extract_connect_list_async(cc,path,dsn_path,):
...
def extract_connect_list(path, dsn_path):
    ...
    for cc in cc_name:
        pool.apply_async(extract_connect_list_async,(cc,path,dsn_path,))
    pool.close()
    pool.join()
...
def groud_truth_based_anova(path, dsn_path, value,debug_path):
    ...
    for _cc in file_name:
        ...
        #     if len(_l)>0:
        #         l[_cc+'-'+asname] = _l
        # groud_truth_based_anova_thread(_cc)
    thread_pool.close()
    thread_pool.join()
...
def country_internal_rank(path,rank_path,_type,cc_list,topo_list,debug_path):
    ...
    for value2 in ['basic', 'user', 'domain']:
        for value in topo_list:
            thread_pool.apply_async(country_internal_rank_thread,(value2,value,))
    thread_pool.close()        
    thread_pool.join()
...
# for rela in ['toposcope', 'toposcope_hidden']:
@record_launch_time
def do_groud_truth_based_anova(prefix,rela,debug_path):
    if debug_path:
    ...
    for value in ['basic', 'user', 'domain']:
        ...
        pool.apply_async(groud_truth_based_anova,(count_path, anova_path, value,debug_path,))
    pool.close()
    pool.join()
...
@record_launch_time
def do_groud_truth_based_var(prefix,rela,debug_path):
    ...
    pool = multiprocessing.Pool(multiprocessing.cpu_count())
    for value in ['basic', 'user', 'domain']:
        pool.apply_async(groud_truth_based_var,(path, value,debug_path,))
    pool.close()
    pool.join()

@record_launch_time
def do_country_internal_rank(path,cc_list,topo_list,debug_path):
